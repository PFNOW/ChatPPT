...
class PPTGenerator:
    def __init__(self):
        self.template_path = None  # 接受模板文件路径
        self.output_path = None  # 接受输出文件路径
        self.powerpoint_data = None  # 接受 PowerPoint 数据

    # 生成 PowerPoint 演示文稿
    def generate_presentation(self, powerpoint_data, template_path: str, output_path: str):
        self.template_path = template_path  # 接受模板文件路径
        self.output_path = output_path  # 接受输出文件路径
        self.powerpoint_data = powerpoint_data  # 接受 PowerPoint 数据

        # 检查模板文件是否存在
        if not os.path.exists(self.template_path):
            LOG.error(f"模板文件 '{self.template_path}' 不存在。")  # 记录错误日志
            raise FileNotFoundError(f"模板文件 '{self.template_path}' 不存在。")

        prs = Presentation(self.template_path)  # 加载 PowerPoint 模板
        remove_all_slides(prs)  # 清除模板中的所有幻灯片
        prs.core_properties.title = self.powerpoint_data.title  # 设置 PowerPoint 的核心标题

        # 遍历所有幻灯片数据，生成对应的 PowerPoint 幻灯片
        for slide in self.powerpoint_data.slides:
        ...
        LOG.info(f"演示文稿已保存到 '{self.output_path}'")
